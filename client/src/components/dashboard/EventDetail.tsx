...
import { RiCalendarLine, RiMapPinLine, RiTimeLine, RiArrowLeftLine, RiGlobalLine, RiShareLine, RiCheckDoubleLine, RiFlashlightLine, RiArrowRightSLine, RiArrowDownSLine } from "react-icons/ri";
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const found = allEvents.find((e: any) => e._id === eventId);
                setEvent(found);
                            ...
                            <div className="pt-6 border-t border-slate-100">
                                {!isPast ? (
                                    <div className="space-y-4">
                                        {/* CASE 1: Luma only */}
                                        {event.lumaUrl && !event.meetupUrl && (
                                            <a href={event.lumaUrl} target="_blank" rel="noopener noreferrer" className="w-full text-base py-5 rounded-2xl flex items-center justify-center bg-slate-900 text-white font-bold hover:bg-slate-800 transition-colors">
                                                Register
                                            </a>
                                        )}
                                        {/* CASE 2: Meetup only */}
                                        {!event.lumaUrl && event.meetupUrl && (
                                            <a href={event.meetupUrl} target="_blank" rel="noopener noreferrer" className="w-full text-base py-5 rounded-2xl flex items-center justify-center bg-slate-900 text-white font-bold hover:bg-slate-800 transition-colors">
                                                Register
                                            </a>
                                        )}
                                        {/* CASE 3: Both Luma and Meetup */}
                                        {event.lumaUrl && event.meetupUrl && (
                                            <div className="relative">
                                                <button
                                                    onClick={() => setShowRegisterMenu(s => !s)}
                                                    onBlur={() => setTimeout(() => setShowRegisterMenu(false), 200)}
                                                    className="w-full text-base py-5 rounded-2xl flex items-center justify-center gap-2 bg-slate-900 text-white font-bold hover:bg-slate-800 transition-colors"
                                                >
                                                    Register <RiArrowDownSLine className={`w-5 h-5 transition-transform ${showRegisterMenu ? 'rotate-180' : ''}`} />
                                                </button>
                                                {showRegisterMenu && (
                                                    <div className="absolute bottom-full left-0 right-0 mb-2 w-full bg-white rounded-2xl shadow-lg border border-slate-100 p-2 z-10">
                                                        <a href={event.lumaUrl} target="_blank" rel="noopener noreferrer" className="block w-full text-left px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-blue-600 rounded-lg">
                                                            Register with Luma
                                                        </a>
                                                        <a href={event.meetupUrl} target="_blank" rel="noopener noreferrer" className="block w-full text-left px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-blue-600 rounded-lg">
