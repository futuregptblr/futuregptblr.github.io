    ...
    >
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="pt-4 mt-4 border-t border-slate-50 flex items-center justify-between">
          ...
          {!isPast && (event.lumaUrl || event.meetupUrl) && (
            <div className="relative" onClick={(e) => e.stopPropagation()}>
              {(event.lumaUrl || event.meetupUrl) && (
                <button onClick={() => navigate(`/events/${(event as any)._id || event.id}`)} className="px-5 py-2 bg-blue-600 text-white rounded-full text-sm font-bold hover:bg-blue-700 transition-colors">
                  Register
                </button>
              )}
            </div>
