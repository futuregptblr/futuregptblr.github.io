...
function signResumeUpload(req, res) {
  try {
    ...
    // Sign only upload parameters that Cloudinary includes in its signature check.
    // resource_type is selected by the /raw/upload endpoint, not signed as a form field.
    const paramsToSign = { 
      folder, 
      timestamp
    };
