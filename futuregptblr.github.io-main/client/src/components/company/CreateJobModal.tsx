...
export function CreateJobModal({ onClose, onJobCreated, job }: CreateJobModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        job
          ? `${API_BASE_URL}/api/company/jobs/${job._id}`
          : `${API_BASE_URL}/api/company/jobs`,
        {
          method: job ? 'PUT' : 'POST',
          headers: {
      ...
      <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
        <div className="mt-3">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium text-gray-900">{job ? 'Edit Job' : 'Post New Job'}</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end space-x-3 pt-4">
              ...
              >
                {loading ? (job ? 'Saving...' : 'Creating...') : (job ? 'Save Changes' : 'Create Job')}
              </button>
            </div>
          </form>
        </div>
